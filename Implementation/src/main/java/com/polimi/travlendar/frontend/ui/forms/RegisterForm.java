...
public class RegisterForm extends FormLayout implements View {
    ...
    /**
     * Through the {@link UserService} class, checks if the email entered is
     * already registered on the database and, if not, registers him.
     */
    private void register() {
        ...
        if (confirmation) {
            if (service.check(input.getEmail())) {
                service.addUser(input); //register user in db
                service.addPreferences(email.getValue()); //init preferences
                user.setUser(service.getUser(input.getEmail(), input.getPassword())); //need to establish current user as autowired User in this session
                try {
